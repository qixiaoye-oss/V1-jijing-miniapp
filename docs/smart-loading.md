...
│  1. 初始白屏                                              │
│     ↓                                                    │
│  2. Shimmer 文字 "正在建立连接..." (登录前)                │
│     ↓                                                    │
│  3. Shimmer 文字 "加载用户数据..." (登录中)                │
│     ↓                                                    │
│  4. Shimmer 文字 "即将完成加载..." (数据加载中)            │
│     ↓                                                    │
│  5. 进度条 + 骨架屏                                       │
│     ↓                                                    │
│  6. 显示真实内容                                          │
...
| 场景 | 处理方式 | 用户感知 |
|------|---------|---------|
| 首次加载 | Shimmer 文字 + 进度条 + 骨架屏 | 有反馈的加载体验 |
...
- `_hideTime` - 后台隐藏时间
- `globalData.loadingStage` - 加载阶段 (connecting → logging → ready)
...
### v1.1.0 (2026-01-02)
- 添加 Shimmer 文字动效加载提示
- app.js 添加 loadingStage 状态管理
- 首页添加三阶段文字提示（连接中 → 登录中 → 加载中）
